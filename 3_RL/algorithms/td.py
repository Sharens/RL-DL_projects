...
def td0_prediction(
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            target = r + (gamma * V[sp] if not done else 0.0)
...
def q_learning(
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            # 1. Wybierz akcję epsilon-greedy (zachowanie eksploracyjne)
            a = epsilon_greedy(Q[s], epsilon, rng)
            ...
            if done:
                # Target dla stanu terminalnego to po prostu nagroda
                target = r
                Q[s, a] += alpha * (target - Q[s, a])
                break

            # 3. Target dla Q-learning: r + gamma * max(Q w stanie sp)
            # Używamy np.max, bo to algorytm off-policy (wybieramy najlepszą opcję)
            target = r + gamma * np.max(Q[sp])

            # 4. Aktualizacja tablicy Q zgodnie ze wzorem TD
            Q[s, a] += alpha * (target - Q[s, a])

            # Przejście do kolejnego stanu
            s = sp

    # Wyciągnięcie ostatecznej polityki zachłannej
    pi_greedy = np.argmax(Q, axis=1)
    return Q, pi_greedy